        ...
        <table cellpadding="5" class="table-visitor">
            ...
            @foreach ( $data as $v )
                <tr class="table-content">
                    ...
                    <td class="table-content" >{{$v->troubleshootBmId->work}}</td>
                    <td class="table-content" >{{ Carbon\Carbon::parse($v->troubleshootBmId->visit)->format('d-m-Y') }}</td>
                    <td class="table-content" >{{ Carbon\Carbon::parse($v->troubleshootBmId->leave)->format('d-m-Y') }}</td>
                    <td class="table-content">{{ $v->nama }}</td>
                    <td class="table-content">{{ $v->checkin }}</td>
                    <td class="table-content"><img src="{{ public_path("storage/bm/troubleshoot/checkin". '/' . $v->photo_checkin) }}" alt="" style="width: 100px; height: 100px;"></td>
                    <td class="table-content">{{ $v->checkout }}</td>
                    <td class="table-content"><img src="{{ public_path("storage/bm/troubleshoot/checkout". '/' . $v->photo_checkout) }}" alt="" style="width: 100px; height: 100px;"></td>
                </tr>
            @endforeach
        </table>
    </main>
